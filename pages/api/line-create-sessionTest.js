// /workspaces/stripe-product-mapper/pages/api/line-create-sessionTest.js
import Stripe from 'stripe';
import { lineItemsMapTests } from '../../src/utils/line-items-MapTests.js';
...
export default async function handler(req, res) {
  try {
    console.log('Starting line-create-sessionTest handler with query:', req.query);
    ...
    const bundleProductIds = lineItemsMapTests[productId] || [];
    console.log(`Bundle product IDs: ${bundleProductIds}`);

    const lineItems = [
      { price: priceId, quantity: 1 },
    ];
    for (const bundleProductId of bundleProductIds) {
      console.log(`Fetching price for bundle product: ${bundleProductId}`);
      const prices = await stripe.prices.list({ product: bundleProductId, active: true, limit: 1 });
      console.log(`Price response for ${bundleProductId}:`, prices.data);
    ...
    console.log(`Bundle items:`, lineItems);
