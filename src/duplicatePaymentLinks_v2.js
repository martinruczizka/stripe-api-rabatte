...
async function duplicatePaymentLinks() {
  ...
  for (const link of allLinks.data) {
    ...
    // 🔍 Prüfen, ob schon ein 50%-Link existiert
    const existing50 = allLinks.data.find(l =>
      l.url.includes("50") && l.metadata?.discount === "50%" && l.metadata?.duplicated_from === link.id
    );

    try {
      if (existing50) {
        // 🛠️ Update statt Neuanlage
  ...
  console.log("\n🎉 Fertig! Alle 50%-Links wurden aktualisiert oder neu erstellt.");
}
